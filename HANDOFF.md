...
- **`direction.yml` executed for the first time on 2026-08-17 (run 31983796753) and was killed at 94%. 360 minutes is the platform's cap, not ours, so the ceiling cannot be raised.** Verified 2026-08-17 against GitHub's own limits reference: *"Each job in a workflow can run for up to 6 hours of execution time. If a job reaches this limit, the job is terminated and fails"* — https://docs.github.com/en/actions/reference/actions-limits, and the same sentence at /en/actions/reference/limits. 360 is also the default for `timeout-minutes`, so the line in the workflow restates the cap rather than choosing it; the two coincide, which is why the cancellation cannot be attributed to one rather than the other, and why that attribution does not matter. **Whatever `timeout-minutes` says, a hosted runner terminates the job at six hours.** The job's own clock agrees to the second: started 01:03:23Z, completed 07:03:38Z — six hours and fifteen seconds, the fifteen being how long the kill took.
  ...
  - **The `if: always()` upload published a decoy, and this is the part to fix first.** `aurex direction` writes `public-data/direction.json` once, at the end. A cancelled run therefore leaves the *checked-out* copy in the workspace, and the upload ships that: the artifact from this run is byte-identical to the one already committed on `main` (sha256 `f1a34f34…`). `if-no-files-found: error` protects nothing, because the file exists. An artifact named `direction` from a run that computed nothing is indistinguishable from one that computed everything unless you hash it — the §1 signature failure exactly. Cheapest honest repair: delete `public-data/direction.json` before the run so the guard has something real to fail on. **Checked, and it is not confined to this workflow:** `shootout.yml` (`public-data/benchmarks.json`) and `factors.yml` (`public-data/factors.json`) upload committed paths on `if: always()` with the same useless `if-no-files-found: error`, so a failed run of either ships the repository's existing artifact under the run's name. `nightly.yml` is unaffected — it commits rather than uploading.
