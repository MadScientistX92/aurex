...
- **`direction.yml` on this branch has never been runnable, and would fail in its first minute.** Commit `2f824fc` made the job resolve offline from `AUREX_CACHE_DIR`, with `cache_dir` defaulting to `tests/fixtures/bench-cache` — and **that directory has never existed in this repository** (`git log --all --diff-filter=A` finds nothing; the only occurrence of the string anywhere is the default value itself). The pre-flight step would exit with "no cached xauusd at tests/fixtures/bench-cache" before any compute. The committed fixture that does exist, `tests/fixtures/seed-cache`, holds 300 rows from 2025-05-21 and would be refused by the same step's two remaining checks — it starts after `--from 2015-01-01`, and its 434-day span is far short of the 500 observations the volatility models need. So the offline redesign is **untested end to end**, and the only `direction.yml` run that has ever happened used `main`'s live-resolution version. Either commit a cache that covers the window (§6's rules permit it for this job — it applies no freshness guard — but note the STOP list of 2026-08-17 forbade committing LBMA history, so this is a maintainer decision, not a cleanup), or make the job able to resolve live when no cache is named.
