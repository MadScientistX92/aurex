...
**The miss: at ten sessions the equal-count screen rejects on both runs, and this is published as a finding because the pre-registration said it would be.** *p* = 0.036 on the full sample and *p* = 0.025 thinned. HAR-RV rejects marginally on both (*p* = 0.0069 and 0.0070) with a resolution of 0.011 against a null of 0.004; AutoARIMA rejects on the thinned run only. `distinguishable_from_zero_equal_count` is `true` at that horizon and nowhere else.

**Four reasons it does not amount to "HAR-RV can call ten-day gold direction" — and all four are reasoning done *after* seeing the number, which is exactly the move that has to be labelled rather than performed smoothly:**
...
- The two runs do not agree on who did it. The full sample names HAR-RV, the thinned sample names AutoARIMA. Every other test in this repository requires both runs to agree before a result reads as one, and the equal-count pair here does not.
- **Multiplicity across horizons is not corrected anywhere in this table.** The screen handles the six models; nothing handles the five horizons, and ten joint decisions at 5% produce about half a rejection by chance. Step 6 already published this exact pattern — AutoARIMA at *p* = 0.029 attached to a skill score of +0.05%, "a coin landing on its edge". Adding a horizon correction now, having seen which cell it would erase, would be inventing a standard to dispose of an inconvenient number, so it is named as a limitation and the number is left standing.
- Taken at face value it is small: 0.011 of resolution against an uncertainty of 0.248 is about 4% of the event's variance, from the model that lost the CRPS shootout by the widest margin of the five at that horizon.

**What would settle it** is a horizon grid this run does not have and a second asset, neither of which is a reason to withhold the rejection now. The claim this repository is entitled to make is the one at the top of this section: resolution is indistinguishable from zero at four of five horizons on both binnings, and at the fifth one binning rejects with the two runs disagreeing about the source.
